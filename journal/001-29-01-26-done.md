...
Added word-count-based reading time calculation to blog post header. Counts words from raw markdown, divides by 200 WPM, displays as "X min read" in the metadata row next to the publication date with a middot separator.

~ src/layouts/BlogPostLayout.astro

---

## Projects showcase section with real GitHub projects

Added Projects section to homepage between Recent Posts and Work Experience. 2-column responsive grid (single column on mobile) with 6 curated projects from GitHub: gtasks (126 stars, Go CLI), mcp-memory-custom (MCP server), QwikScan (hackathon winner, Flutter), covid-twitter-telegram-bot (Go), genlookup (Chrome extension, LLM), portfolio-v2 (this site).

Cards feature monospace project names, amber star counts, "Hackathon Winner" pill badge, tech tag chips, hover lift with translateY, and staggered fade-in animation. `ProjectEntry` interface in content.ts with name, description, url, tech array, optional stars and highlight fields.

~ src/lib/content.ts (added ProjectEntry interface + projects array)
~ src/pages/index.astro (Projects section + project-grid CSS)
~ CLAUDE.md (updated context)

---

## Blog prose styling fixes

Fixed image paths in blog markdown (relative → absolute for public/ assets). Added `figure`/`figcaption` styling: border, theme-aware shadow, centered italic sans-serif caption. Constrained prose images to `max-width: 75%` and `max-height: 28rem` with `object-fit: contain`. Fixed invisible list markers — added `list-style-type: disc` for `ul` and `decimal` for `ol`.
